...
- [`sf force analytics template create`](#sf-force-analytics-template-create)
- [`sf force apex class create`](#sf-force-apex-class-create)
- [`sf force apex trigger create`](#sf-force-apex-trigger-create)
- [`sf force lightning app create`](#sf-force-lightning-app-create)
- [`sf force lightning component create`](#sf-force-lightning-component-create)
- [`sf force lightning event create`](#sf-force-lightning-event-create)
- [`sf force lightning interface create`](#sf-force-lightning-interface-create)
- [`sf force lightning test create`](#sf-force-lightning-test-create)
- [`sf force project create`](#sf-force-project-create)
- [`sf force staticresource create`](#sf-force-staticresource-create)
- [`sf force visualforce component create`](#sf-force-visualforce-component-create)
- [`sf force visualforce page create`](#sf-force-visualforce-page-create)
...
```
USAGE
  $ sf force analytics template create -n <value> [--json] [--flags-dir <value>] [-d <value>] [--api-version <value>]
...
EXAMPLES
  ...
  force-app/main/default/waveTemplates directory:
    ...
    $ sf force analytics template create --name myTemplate --output-dir force-app/main/default/waveTemplates
...
```
USAGE
  $ sf force apex class create -n <value> [--json] [--flags-dir <value>] [-t
    ApexException|ApexUnitTest|BasicUnitTest|DefaultApexClass|InboundEmailService] [-d <value>] [--api-version <value>]
...
EXAMPLES
  ...
  current directory:
    ...
    $ sf force apex class create --name MyClass

  Similar to previous example, but generates the files in the "force-app/main/default/classes" directory:

    $ sf force apex class create --name MyClass --output-dir force-app/main/default/classes
...
```
USAGE
  $ sf force apex trigger create -n <value> [--json] [--flags-dir <value>] [-t ApexTrigger] [-d <value>] [--api-version
    <value>] [-s <value>] [-e before insert|before update|before delete|after insert|after update|after delete|after
    undelete...]
...
EXAMPLES
  ...
  the current directory:
    ...
    $ sf force apex trigger create --name MyTrigger

  Similar to the previous example, but generate the files in the "force-app/main/default/triggers" directory:

    $ sf force apex trigger create --name MyTrigger --output-dir force-app/main/default/triggers

  Generate files for a trigger that fires on the Account object before and after an insert:

    $ sf force apex trigger create --name MyTrigger --sobject Account --event "before insert,after insert"
...
```
USAGE
  $ sf force lightning app create -n <value> [--json] [--flags-dir <value>] [-t DefaultLightningApp] [-d <value>] [--api-version
    <value>]
...
EXAMPLES
  Generate the metadata files for a Lightning app bundle called "myapp" in the current directory:
    ...
    $ sf force lightning app create --name myapp

  Similar to the previous example, but generate the files in the "force-app/main/default/aura" directory:

    $ sf force lightning app create --name myapp --output-dir force-app/main/default/aura
...
```
USAGE
  $ sf force lightning component create -n <value> [--json] [--flags-dir <value>] [-t
...
EXAMPLES
  Generate the metadata files for an Aura component bundle in the current directory:
    ...
    $ sf force lightning component create --name mycomponent

  Generate a Lightning web component bundle in the current directory:

    $ sf force lightning component create --name mycomponent --type lwc

  Generate an Aura component bundle in the "force-app/main/default/aura" directory:

    $ sf force lightning component create --name mycomponent --output-dir force-app/main/default/aura

  Generate a Lightning web component bundle in the "force-app/main/default/lwc" directory:

    $ sf force lightning component create --name mycomponent --type lwc --output-dir force-app/main/default/lwc
...
```
USAGE
  $ sf force lightning event create -n <value> [--json] [--flags-dir <value>] [-t DefaultLightningEvt] [-d <value>] [--api-version
    <value>]
...
EXAMPLES
  Generate the metadata files for a Lightning event bundle called "myevent" in the current directory:
    ...
    $ sf force lightning event create --name myevent

  Similar to previous example, but generate the files in the "force-app/main/default/aura" directory:

    $ sf force lightning event create --name myevent --output-dir force-app/main/default/aura
...
```
USAGE
  $ sf force lightning interface create -n <value> [--json] [--flags-dir <value>] [-t DefaultLightningIntf] [-d <value>]
    [--api-version <value>]
...
EXAMPLES
  Generate the metadata files for a Lightning interface bundle called "myinterface" in the current directory:
    ...
    $ sf force lightning interface create --name myinterface

  Similar to the previous example but generate the files in the "force-app/main/default/aura" directory:

    $ sf force lightning interface create --name myinterface --output-dir force-app/main/default/aura
...
```
USAGE
  $ sf force lightning test create -n <value> [--json] [--flags-dir <value>] [-t DefaultLightningTest] [-d <value>]
    [--api-version <value>]
...
EXAMPLES
  Generate the metadata files for the Lightning test called MyLightningTest in the current directory:
    ...
    $ sf force lightning test create --name MyLightningTest

  Similar to the previous example but generate the files in the "force-app/main/default/lightningTests" directory:

    $ sf force lightning test create --name MyLightningTest --output-dir force-app/main/default/lightningTests
...
```
USAGE
  $ sf force project create -n <value> [--json] [--flags-dir <value>] [-t standard|empty|analytics|reactb2e|reactb2x] [-d
    <value>] [-s <value>] [-p <value>] [-x] [--lwc-language javascript|typescript] [--api-version <value>]
...
EXAMPLES
  Generate a project called "mywork":
    ...
    $ sf force project create --name mywork

  Similar to previous example, but generate the files in a directory called "myapp":

    $ sf force project create --name mywork --default-package-dir myapp

  Similar to prevoius example, but also generate a default package.xml manifest file:

    $ sf force project create --name mywork --default-package-dir myapp --manifest

  Generate a project with the minimum files and directories:

    $ sf force project create --name mywork --template empty

  Generate a TypeScript project for Lightning Web Components:

    $ sf force project create --name mywork --lwc-language typescript
...
```
USAGE
  $ sf force staticresource create -n <value> [--json] [--flags-dir <value>] [--type <value>] [-d <value>]
  [--api-version <value>]
...
EXAMPLES
  Generate the metadata file for a static resource called MyResource in the current directory:
    ...
    $ sf force staticresource create --name MyResource

  Similar to previous example, but specifies a MIME type of application/json:

    $ sf force staticresource create --name MyResource --type application/json

  Generate the resource file in the "force-app/main/default/staticresources" directory:

    $ sf force staticresource create --name MyResource --output-dir force-app/main/default/staticresources
...
```
USAGE
  $ sf force visualforce component create -n <value> -l <value> [--json] [--flags-dir <value>] [-t DefaultVFComponent] [-d <value>]
    [--api-version <value>]
...
EXAMPLES
  Generate the metadata files for a Visualforce component in the current directory:
    ...
    $ sf force visualforce component create --name mycomponent --label mylabel

  Similar to previous example, but generate the files in the directory "force-app/main/default/components":

    $ sf force visualforce component create --name mycomponent --label mylabel --output-dir components
...
```
USAGE
  $ sf force visualforce page create -n <value> -l <value> [--json] [--flags-dir <value>] [-d <value>] [--api-version
  <value>]
...
EXAMPLES
  Generate the metadata files for a Visualforce page in the current directory:
    ...
    $ sf force visualforce page create --name mypage --label mylabel

  Similar to previous example, but generate the files in the directory "force-app/main/default/pages":

    $ sf force visualforce page create --name mypage --label mylabel --output-dir pages
...
```
USAGE
  $ sf project generate -n <value> [--json] [--flags-dir <value>] [-t standard|empty|analytics|reactb2e|reactb2x] [-d
    <value>] [-s <value>] [-p <value>] [-x] [--lwc-language javascript|typescript] [--api-version <value>]
...
EXAMPLES
  ...
  Generate a TypeScript project for Lightning Web Components:
    ...
    $ sf project generate --name mywork --lwc-language typescript
...
_See code: [src/commands/template/generate/analytics/template.ts](https://github.com/salesforcecli/plugin-templates/blob/v56.10.2/src/commands/template/generate/analytics/template.ts)_
...
_See code: [src/commands/template/generate/apex/class.ts](https://github.com/salesforcecli/plugin-templates/blob/v56.10.2/src/commands/template/generate/apex/class.ts)_
...
_See code: [src/commands/template/generate/apex/trigger.ts](https://github.com/salesforcecli/plugin-templates/blob/v56.10.2/src/commands/template/generate/apex/trigger.ts)_
...
_See code: [src/commands/template/generate/digital-experience/site.ts](https://github.com/salesforcecli/plugin-templates/blob/v56.10.2/src/commands/template/generate/digital-experience/site.ts)_
...
_See code: [src/commands/template/generate/flexipage/index.ts](https://github.com/salesforcecli/plugin-templates/blob/v56.10.2/src/commands/template/generate/flexipage/index.ts)_
...
_See code: [src/commands/template/generate/lightning/app.ts](https://github.com/salesforcecli/plugin-templates/blob/v56.10.2/src/commands/template/generate/lightning/app.ts)_
...
_See code: [src/commands/template/generate/lightning/component.ts](https://github.com/salesforcecli/plugin-templates/blob/v56.10.2/src/commands/template/generate/lightning/component.ts)_
...
_See code: [src/commands/template/generate/lightning/event.ts](https://github.com/salesforcecli/plugin-templates/blob/v56.10.2/src/commands/template/generate/lightning/event.ts)_
...
_See code: [src/commands/template/generate/lightning/interface.ts](https://github.com/salesforcecli/plugin-templates/blob/v56.10.2/src/commands/template/generate/lightning/interface.ts)_
...
_See code: [src/commands/template/generate/lightning/test.ts](https://github.com/salesforcecli/plugin-templates/blob/v56.10.2/src/commands/template/generate/lightning/test.ts)_
...
_See code: [src/commands/template/generate/project/index.ts](https://github.com/salesforcecli/plugin-templates/blob/v56.10.2/src/commands/template/generate/project/index.ts)_
...
_See code: [src/commands/template/generate/static-resource/index.ts](https://github.com/salesforcecli/plugin-templates/blob/v56.10.2/src/commands/template/generate/static-resource/index.ts)_
...
_See code: [src/commands/template/generate/visualforce/component.ts](https://github.com/salesforcecli/plugin-templates/blob/v56.10.2/src/commands/template/generate/visualforce/component.ts)_
...
_See code: [src/commands/template/generate/visualforce/page.ts](https://github.com/salesforcecli/plugin-templates/blob/v56.10.2/src/commands/template/generate/visualforce/page.ts)_
...
```
USAGE
  $ sf webapp generate -n <value> [--json] [--flags-dir <value>] [-t default|reactbasic] [-l <value>] [-d <value>]
    [--api-version <value>]

FLAGS
  -d, --output-dir=<value>   Directory for saving the created files.
  -l, --label=<value>        Master label for the web application.
  -n, --name=<value>         (required) Name of the generated web application.
  -t, --template=<option>    [default: default] Template to use for file creation.
...
DESCRIPTION
  ...
  Generates a web application in the specified directory or the current working directory. The web application files are
  created in a folder with the designated name. Web application files must be contained in a parent directory called
  "webapplications" in your package directory. Either run this command from an existing directory of this name, or use
  the --output-dir flag to create one or point to an existing one.
...
EXAMPLES
  Generate a web application called MyWebApp in the current directory:
    ...
    $ sf webapp generate --name MyWebApp

  Generate a React-based web application:

    $ sf webapp generate --name MyReactApp --template reactbasic

  Generate the web application in the "force-app/main/default/webapplications" directory:

    $ sf webapp generate --name MyWebApp --output-dir force-app/main/default/webapplications
...
FLAG DESCRIPTIONS
  -d, --output-dir=<value>  Directory for saving the created files.
    ...
    **Important:** The generator automatically ensures the output directory ends with "webapplications". If your
    specified path doesn't end with "webapplications", it's automatically appended. The web application is created at
    "<output-dir>/<webappname>".

    **Examples:**
    - "--output-dir force-app/main/default" → Creates a web application at
    "force-app/main/default/webapplications/MyWebApp/"
    - "--output-dir force-app/main/default/webapplications" → Creates a web application at
    "force-app/main/default/webapplications/MyWebApp/" (no change)

    If not specified, the command reads your sfdx-project.json and defaults to "webapplications" directory within your
    default package directory. When running outside a Salesforce DX project, defaults to the current directory.

  -l, --label=<value>  Master label for the web application.

    If not specified, the label is derived from the name.

  -n, --name=<value>  Name of the generated web application.
...
This plugin supports creating TypeScript projects for Lightning Web Components. Use the `--lwc-language typescript` flag with `sf template generate project` to create a project configured for TypeScript development.
...
sf template generate project --name my-typescript-project --lwc-language typescript
...
# Compile TypeScript locally for validation
npm run build

# Deploy to Salesforce (deploys raw .ts files)
sf project deploy start
```

For more information, see the `--lwc-language` flag documentation in the `sf template generate project` command above.
