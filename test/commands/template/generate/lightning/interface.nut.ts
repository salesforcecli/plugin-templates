...
describe('template generate lightning interface:', () => {
  ...
  describe('Check lightning interface creation', () => {
    it('should create lightning interface foo using DefaultLightningIntf template and aura output directory', () => {
      execCmd(
        'template generate lightning generate interface --interfacename foo --outputdir aura --template DefaultLightningIntf',
        {
          ensureExitCode: 0,
        }
      );
    ...
    it('should create lightning interface foo using DefaultLightningIntf template and aura output directory and no -meta.xml file', () => {
      execCmd(
        'template generate lightning generate interface --interfacename foometa --outputdir aura --template DefaultLightningIntf --internal',
        { ensureExitCode: 0 }
      );
      ...
      it('should create lightning interface foo using DefaultLightningIntf template and custom output directory', () => {
        execCmd(
          `template generate lightning generate interface --interfacename foo --outputdir ${path.join(
            'aura',
            'interfacetest'
  ...
  describe('lightning interface failures', () => {
    it('should throw invalid template name error', () => {
      const stderr = execCmd(
        'template generate lightning generate interface --interfacename foo --outputdir aura --template foo'
      ).shellOutput.stderr;
      expect(stderr).to.contain(messages.getMessage('InvalidTemplate'));
    });

    it('should throw missing aura parent folder error', () => {
      const stderr = execCmd('template generate lightning generate interface --interfacename foo').shellOutput.stderr;
      expect(stderr).to.contain(messages.getMessage('MissingAuraFolder'));
    });

    it('should throw missing interfacename error', () => {
      const stderr = execCmd('template generate lightning generate interface --outputdir aura').shellOutput.stderr;
      expect(stderr).to.contain('Missing required flag');
    });

    it('should throw invalid non alphanumeric interfacename error', () => {
      const stderr = execCmd('template generate lightning generate interface --interfacename /a --outputdir aura')
        .shellOutput.stderr;
    ...
    it('should throw invalid interfacename starting with numeric error', () => {
      const stderr = execCmd('template generate lightning generate interface --interfacename 3aa --outputdir aura')
        .shellOutput.stderr;
    ...
    it('should throw invalid interfacename ending with underscore error', () => {
      const stderr = execCmd('template generate lightning generate interface --interfacename a_ --outputdir aura')
        .shellOutput.stderr;
    ...
    it('should throw invalid interfacename with double underscore error', () => {
      const stderr = execCmd('template generate lightning generate interface --interfacename a__a --outputdir aura')
        .shellOutput.stderr;
